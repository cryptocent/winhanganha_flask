# First Nations Winhanganha Archive Flask Project

## Project Structure

```text
.env
run.py
project/
  __init__.py        Flask app, Bootstrap, MySQL and Flask-Login setup
  views.py           Routes and page handlers
  models.py          Database helper functions, user model and login loader
  decorators.py      Custom decorators and permission checks
  forms.py           WTForms classes
  templates/         Jinja templates
  static/            CSS and images
winhanganha_archive.sql
```

## Setup

### 1. Create and activate a virtual environment

```bash
python3 -m venv venv
```

#### Windows

```bash
venv/scripts/activate
```

#### Linux

```bash
source venv/bin/activate
```

### 2. Install dependencies

```bash
python3 -m pip install -r requirements.txt
```

### 3. Create a `.env` file beside `run.py`

```env
SECRET_KEY=replace_with_a_long_random_secret
MYSQL_HOST=localhost
MYSQL_PORT=3306
MYSQL_USER=root
MYSQL_PASSWORD=your_mysql_password
MYSQL_DATABASE=winhanganha_archive
FLASK_HOST=127.0.0.1
FLASK_PORT=5000
FLASK_DEBUG=true
```

### 4. Create the database

Import the database file into MySQL:

```text
winhanganha_archive.sql
```

Example command:

```bash
mysql -u root -p winhanganha_archive < winhanganha_archive.sql
```

### 5. Run the app

```bash
python3 run.py
```

## Notes

* Keep the `.env` file out of version control.
* Ensure MySQL is running before starting the Flask application.
* Confirm the database name in `.env` matches the imported database.
* Use `requirements.txt` to keep dependency installation consistent across environments.
